function fn_list_endpoints()
{
    ...
    $(".loading").show();
    $.getJSON("/api/v1/dashboard/endpoints", {queryStr: qs, tags: tags, limit:limit, _r:Math.random()}, function(ret){
                $(".loading").hide();
                if (!ret.ok) {
                    alert(ret.msg);
                    return;
                }
                var hosts = ret.data.endpoints;
                ...
                for (var hidx in hosts) {
                    var h = hosts[hidx];
                    var  line_html = '<tr>'
                         + '<td><label><input type="checkbox" class="input shiftCheckbox" checked="checked" data-fullname="' + h.endpoint + '">'+'<span class="text"></span></label></td>'
                         + '<td><span class="text">' + h.endpoint + '</span></td>'
                         + '</tr>'
...
function fn_list_counters(){
    ...
    $.ajax({
        method: "POST",
        url: "/api/v1/dashboard/counters",
        dataType: "json",
        data: {"endpoints": JSON.stringify(hosts), "queryStr": qs, "limit": limit, "_r": Math.random()},
        success:function(ret){
            $(".loading").hide();
            if(ret.ok){
                var items = ret.data.counters;
                // display counters
                ...
                for (var i in items) {
                    ...
                    var line_html = '<tr>'
                    + '<td><label><input type="checkbox" class="input shiftCheckbox" data-fullkey="'+c.Counter+'"><span class="text"></span></label></td>'
                    + '<td><a href="javascript:void(0);" onclick="fn_show_chart(\'' + c.Counter + '\')" >' + c.Counter + '</a></td>'
